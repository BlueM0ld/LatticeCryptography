...
def compute_and_plot_gso(M):
    ...
    log_gso_norms = [RR(log(square_gso_norm, _sage_const_2)/_sage_const_2)
...
def generate_shifted_polynomials(polynomial, N, m, d):
    ...
    for i in range(m + _sage_const_1):
        ...
        for shift in shifts:
            g = base * prod(v**s for v,
                            s in zip(polynomial.variables(), shift))
            G.append(g)
...
def small_roots(polynomial, bounds, m=_sage_const_1, d=None):
    ...
    if isinstance(polynomial, Polynomial):
        x, = polygens(polynomial.base_ring(),
                      polynomial.variable_name(), _sage_const_1)
    ...
    lattice_basis_mtrx, monomials = rescale_and_reduce_matrix(
        lattice_basis_mtrx, monomials, bounds)
...
def coppersmith_univariate(N, c, known_prefix, unknown_length, e):
    ...
    P = PolynomialRing(Zmod(N), _sage_const_1, names=('x',))
    (x,) = P._first_ngens(1)
    ...
    if roots:
        ...
        recovered_message_str = Integer(
            recovered_message).str(base=_sage_const_35)
